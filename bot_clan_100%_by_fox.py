...
[11EAFD][b][c]
°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°
Failed to fetch information. Try again later!!
...
class FF_CLIENT(threading.Thread):
    ...
    def joinclanchat(self):
        fields = {
            1: 3,
            2: {
                1: 3075740690,#clan id
                2: 1,
                4: str("L8COTJ+8rk0osuIL1rnLRw"),#clan key
            }
        }
        packet = create_protobuf_packet(fields)
        packet = packet.hex()
    ...
    def connect(self, tok, host, port, packet, key, iv):
        ...
        while True:
            ...
            if senthi == True:
                ...
                clients.send(
                        self.GenResponsMsg(
                            f"""[C][B]Peace be upon you, how are you? Thank you for accepting my friend request. Send an emoji to find out what you need to do.""", idinv
                        )
                )
                senthi = False            
                ...
                if "8" in parsed_data["5"]["data"] and "data" in parsed_data["5"]["data"]["8"]:
                    ...
                    if uexmojiii == "DefaultMessageWithKey":
                        ...
                        clients.send(
                            self.GenResponsMsg(
                                f"""[C][B]
Welcome to the clan [00ff00]CODEX HAT[ffffff]!

[C][B]
If you want to know the commands, just send: [00ff00]🗿/🗿help🗿[ffffff]

[C][B]
Warning! The bot only works for those who have the clan tag: [00ff00]CDX[ffffff]
"""
                            )
                        )
                else:
...
####################################
            ...
            if "1200" in data.hex()[0:4] and b"/3s" in data:
                ...
                if "CDX" not in sender_name:
	                    clients.send(
            self.GenResponsMsg(
                f"[C][B][FF0000]You are not tagged!\n[ffffff]You must tag the clan to have the right to use the bot."
            )
        )
                else:
                    packetmaker = self.skwad_maker()
...
####################################
            ...
            self.GenResponsMsg(
                ...
                """))
                    ...
                    try:
                        ...
                        parsed_data = json.loads(json_result)
                        uid = parsed_data["5"]["data"]["1"]["data"]
                        error_msg = f"[FF0000]mistake: {e}" if "ID" in str(e) else f"[FF0000]Error fetching player: {e}"
            ...
            self.GenResponsMsg(
                ...
                [11EAFD][b][c]
                °°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°
                Spamming player join requests has started:
                {iddd_display}
                °°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°
                [FFB300][b][c]BOT MADE BY CODEX TEAM
                """))
